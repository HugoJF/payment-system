...
const UniverseMax = 5;
     ...
     */
    const TypeInvalid = 0;
        const TypeIndividual = 1; // Max universes, not an actual universe
...
const InstanceFlagMMSLobby = 131072; // ( k_unSteamAccountInstanceMask + 1 ) >> 1
        /**
     * Vanity URL types used by ResolveVanityURL method.
     */
    const VanityIndividual = 1; // ( k_unSteamAccountInstanceMask + 1 ) >> 2
        const VanityGroup = 2; // ( k_unSteamAccountInstanceMask + 1 ) >> 3
    ...
    public static function SetFromURL($Value, callable $VanityCallback)
    {
        if (preg_match('/^https?:\/\/steamcommunity\.com\/profiles\/(.+?)(?:\/|$)/', $Value, $Matches) === 1) {
            ...
            if (preg_match('/^https?:\/\/steamcommunity\.com\/(id|groups|games)\/([\w-]+)(?:\/|$)/', $Value, $Matches) === 1
                || preg_match('/^()([\w-]+)$/', $Value, $Matches) === 1) // Empty capturing group so that $Matches has same indexes
            {
                ...
                if (preg_match('/^https?:\/\/(steamcommunity\.com\/user|s\.team\/p)\/([\w-]+)(?:\/|$)/', $Value, $Matches) === 1) {
    ...
    public static function normalizeSteamID($steamID64)
    {
        try {
        ...
        switch ($AccountType) {
            case self::TypeChat:
            {
                if ($AccountInstance & SteamID :: InstanceFlagClan) {
                    ...
                    if ($AccountInstance & SteamID :: InstanceFlagLobby) {
                        $AccountTypeChar = 'L';
                    }
                }
